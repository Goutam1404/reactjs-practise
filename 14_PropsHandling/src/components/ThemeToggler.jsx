...
const ThemeContext = createContext(); //This creates a container

//PROVIDER: This is just a function
...
function ThemeCard({ title, children }) {
  ...
  return (
    <div
      className={`${
        isDark ? "bg-neutral-950 text-white" : "bg-white text-gray-900"
      } p-2`}
    >
      <h3>{title}</h3>
      <div>{children}</div>
...
function ThemedButton({children, variant='primary', onClick
  ...
  const getButtonClasses=()=>{
    if(variant==='primary'){
      return isDark?"bg-blue-600 hover:bg-blue-700 text-white":
      "bg-orange-500 hover:bg-orange-600 text-white"
    }
    if(variant ==='secondary'){
      return isDark ?"bg-gray-700 hover:bg-gray-600 text-white":
